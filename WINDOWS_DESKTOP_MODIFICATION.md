...
本文记录把源项目 harness 的 **Web 界面原封不动**打包成 Windows 桌面程序（双击即用、无需 cmd）的完整方案、构建方法与产物。当前版本 **0.3.1**，基于上游 `dsh-v0.1.1-rc.2`。
...
- 单文件安装包：`.artifacts/desktop-web/DeepSeekHarnessWeb-Setup-0.3.1-x64.exe`（约 90MB，Inno Setup 7；默认装到 %LOCALAPPDATA%\Programs\DeepSeek Harness）。
...
GitHub Release `v0.3.1`：资产 `DeepSeekHarnessWeb-Setup-0.3.1-x64.exe`。历史版本：`v0.2.0`、`v0.2.1`。
