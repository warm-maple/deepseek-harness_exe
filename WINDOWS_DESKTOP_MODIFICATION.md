...
- 双击 `DeepSeekHarnessWeb.exe` → 自动找空闲端口 → 启动内置 Node 运行原版 `dsh web --port <port>` → 轮询就绪 → WebView2 加载 `http://127.0.0.1:<port>` → 关闭窗口自动 `taskkill /T /F` 结束整棵 Node 进程树。
...
- 应用图标：`apps/desktop-web/app.ico`（用户提供 PNG 转多尺寸 ICO；exe 图标经 ApplicationIcon 嵌入，窗口图标经 AppContext.BaseDirectory 加载）。
